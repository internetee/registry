$ ->
  birthdayRestrictedCountries = window.BIRTHDAY_RESTRICTED_COUNTRIES || []

  $('.js-contact-form').on 'restoreDefault', (e) ->
    form = $(e.target)
    form.find('.js-ident-tip').hide()
    selectedCountry = $('.js-ident-country-code option:selected').val()
    isBirthdayRestrictedCountry = birthdayRestrictedCountries.includes(selectedCountry)
    $('.js-ident-type').find('option[value=birthday]').prop('disabled', isBirthdayRestrictedCountry)
