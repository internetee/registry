class LegalDocument < ActiveRecord::Base
  ...
  TYPES = if ENV['legal_document_types'].present?
            ENV['legal_document_types'].split(',').map(&:strip)
          else
            %w[pdf asice asics sce scs adoc edoc bdoc ddoc zip rar gz tar 7z odt
               doc docx].freeze
          end
  ...
  def save_to_filesystem
    ...
    loop do
      ...
      FileUtils.mkdir_p(dir, mode: 0o775)
  ...
  def self.remove_duplicates
    ...
    LegalDocument.where(documentable_type: 'Domain').where.not(checksum: [nil, '']).find_each do |orig_legal|
      ...
      LegalDocument.where(documentable_type: 'Domain', documentable_id: orig_legal.documentable_id)
                   ...
                   .where.not(id: orig_legal.id).where.not(path: orig_legal.path).find_each do |new_legal|
      ...
      LegalDocument.where(documentable_type: 'Contact', documentable_id: contact_ids)
                   .where(checksum: orig_legal.checksum).where.not(path: orig_legal.path).find_each do |new_legal|
        next if modified.include?(orig_legal.id)
