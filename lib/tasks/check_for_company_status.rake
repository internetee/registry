...
namespace :company_status do
  # bundle exec rake company_status:check_all -- --days_interval=128 --spam_time_delay=0.3 --batch_size=100 --force_delete=false
  ...
  task check_all: :environment do
    ...
    opts_hash = {
      ...
      force_delete: ["--force_delete=VALUE", String]
    }
    ...
    options[:force_delete] = options[:force_delete].downcase == 'true'
    CompanyRegisterStatusJob.perform_later(options[:days_interval], options[:spam_time_delay], options[:batch_size], options[:force_delete])
  end  
end
